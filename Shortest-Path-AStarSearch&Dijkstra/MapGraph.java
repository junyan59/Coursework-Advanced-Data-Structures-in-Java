...
public class MapGraph {
    ...
    public List<GeographicPoint> dijkstra(GeographicPoint start,
        ...
        PriorityQueue<MapNodeAndDistance> toExplore = new PriorityQueue<>();
        HashSet<MapNode> visited = new HashSet<>();
        HashMap<MapNode, Double> distanceMap = new HashMap<>();
        MapNodeAndDistance startNodeAndDistance = new MapNodeAndDistance(startNode, 0.0);
        toExplore.add(startNodeAndDistance);

        MapNodeAndDistance next = null;

        while (!toExplore.isEmpty()) {
            next = toExplore.remove();

            // hook for visualization
            nodeSearched.accept(next.point.getLocation());

            if (!visited.contains(next.point)) {
                visited.add(next.point);

                if (next.point.equals(endNode)) break;

                Set<MapEdge> edges = next.point.getEdges();
                for (MapEdge edge : edges) {
                    MapNode neighbor = edge.getEndNode();

                    if (!visited.contains(neighbor)) {

                        if (!distanceMap.containsKey(neighbor) || next.distance + edge.getLength() < distanceMap.get(neighbor)) {
                            distanceMap.put(neighbor, next.distance + edge.getLength());
                            parentMap.put(neighbor, next.point);
                            MapNodeAndDistance currNodeAndDistance = new MapNodeAndDistance(neighbor, distanceMap.get(neighbor));
                            toExplore.add(currNodeAndDistance);
                        }
    ...
    public List<GeographicPoint> aStarSearch(GeographicPoint start,
        ...
        HashMap<MapNode, MapNode> parentMap = new HashMap<>();
        PriorityQueue<MapNodeAndTotalDistance> toExplore = new PriorityQueue<>();
        HashSet<MapNode> visited = new HashSet<>();
        HashMap<MapNode, Double> distanceMap = new HashMap<>();
        MapNodeAndTotalDistance startNodeAndEstDistance = new MapNodeAndTotalDistance(startNode, 0.0, startNode.getLocation().distance(goal));
        toExplore.add(startNodeAndEstDistance);

        MapNodeAndTotalDistance next = null;
                ...
                for (MapEdge edge : edges) {
                    ...
                    if (!visited.contains(neighbor)) {
                        ...
                        if (!distanceMap.containsKey(neighbor) || next.distance + edge.getLength() < distanceMap.get(neighbor)) {
                            distanceMap.put(neighbor, next.distance + edge.getLength());
                            parentMap.put(neighbor, next.point);
                            double distanceToGoal = neighbor.getLocation().distance(goal);
                            MapNodeAndTotalDistance currNodeAndDistance = new MapNodeAndTotalDistance(neighbor, distanceMap.get(neighbor), distanceToGoal);
                            toExplore.add(currNodeAndDistance);
                        }

...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for(int i = 0; i < numVertices; i++){
			...
			//find the neighbors of each vertex, count the numbers of neighbors
			// add each degree to a list
			List<Integer> getInNeighbors = getInNeighbors(i);
			List<Integer> getNeighbors = getNeighbors(i);
		    int sumNeighbors = getInNeighbors.size() + getNeighbors.size();
		    degreeList.add(sumNeighbors);
		}
		// sort the list from largest integer to smallest integer
